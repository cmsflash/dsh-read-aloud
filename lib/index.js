...
var ReadAloudService = class extends Service2 {
  ...
  async audio(request) {
    ...
    if (events === void 0) return this.refuse(request, { ok: false, code: "session-not-found" });
    const text = spokenTextOf(events, request.messageId);
    if (text === void 0) return this.refuse(request, { ok: false, code: "message-not-found" });
    try {
   ...
   */
  refuse(request, failure) {
    const detail = failure.detail === void 0 ? "" : `: ${failure.detail}`;
    this.ctx.logger.warn(
      `read-aloud: audio for message ${request.messageId} in session ${request.sessionId} refused (${failure.code})${detail}`
    );
    return failure;
  }
  /**
  ...
  reportPlaybackFailure(report) {
    this.ctx.logger.warn(
      `read-aloud: playback of message ${report.messageId} in session ${report.sessionId} failed at ${report.stage}: ${report.reason}`
    );
  }
  /**
  ...
  async eventsOf(sessionId) {
    ...
    const inspected = await persistence.inspect(sessionId).catch(() => void 0);
    return inspected === void 0 ? void 0 : inspected.events;
  }
  /**
...
var STAGES = ["request", "decode", "play"];
...
function apply(ctx, config) {
  ...
  ctx.inject(["tts", "credentials"], async (scope) => {
    try {
      const credentials = scope.get("credentials");
      const secret = async (ref) => ref === void 0 ? void 0 : (await credentials.resolve(ref))?.value;
